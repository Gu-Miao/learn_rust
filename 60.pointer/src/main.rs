...
//   的风险，就可以获得原始指针的性能。因而，带有安全性保障的引用应该作为首选使用。
// * 访问原始指针总是 unsafe 的。
// * 原始指针不拥有值的所有权，在访问时编译器也不会检查其合法性。
// * Rust 允许多个原始指针指向同一块数据，也就是说 Rust 不能保证数据共享的合法性。
//
// 使用原始指针的情况
// * 一些操作系统或第三方库需要，或与 C 语言交互。
// * 共享某些内容的访问至关重要，且对运行时的性能要求高。

// Rust 指针
//
// * 原始指针 -- *const T 和 *mut T，速度快，擅长与 C 交互；但是非常不安全。
// * Box<T> -- 可以将任何类型放入 Box 中。可接受几乎所有类型的长期存储，在安全编程时代是 Rust 的主力军。
//   可以将值存放于堆内存上；但会增加占用空间。
// * Rc<T> -- 是 Rust 能干而又吝啬的记簿员。他知道谁接了什么，什么时候借的。它擅长处理对值的共享访问；但
//   占用空间增加，并且有运行时成本，且不能在线程间进行共享。
// * Arc<T> -- 在线程间共享值，且保证其安全性。它弥补了 Rc<T> 的不足，专门在线程间共享值而使用；但同样有
//   占用空间增加以及运行时成本的问题。
// * Cell<T> -- 拥有改变不可变值的能力；但大小增加，性能差一些。
// * RefCell<T> -- 对不可变引用执行改变，可与仅接受不可变引用的 Rc、Arc 进行嵌套使用。缺点是大小增加，有
//   运行时成本，且缺乏编译时的保障。
// * Cow<T> -- Copy on Write，封闭并提供对借用数据的不可变访问，并在需要修改值或所有权时，延时克隆数据。
//   优点是当只是只读访问时，避免写入；缺点是大小会增加。
// * String -- 处理可变长度的文本，优点是可以动态按需增长并且在运行时保证正确编码；缺点是可能过度分配内存。
// * Vec<T> -- 十分常见的类型，动态“数组”，优点为动态按需增长，缺点是可能过度分配内存。
